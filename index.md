---
layout: default
title: Home
nav_order: 0
---

# Lost in Space (But Make It Data): Navigating Images with CARTA

This is a basic tutorial on the CARTA image viewer [CARTA webpage](https://cartavis.org/) for the Italian community and users of the Italian ARC node.
This tutorial walks you through the basic things to know to use CARTA for image management and scientific exploitation.

---

## Contents

1. [What is CARTA?](pages/01_introduction.md) — Overview of CARTA
2. [How to access CARTA?](pages/02_before_you_start.md) — Choose the way to access CARTA
3. [Meeting the interface](pages/03_interface.md) — Overview of the initial CARTA interface
4. [Layouts and preferences](pages/04_layouts.md) — Setting Layouts and preferences for widgets
5. [Image management](pages/05_image_management.md) — Image management: open, append, match, save, close
6. [How to define Regions](pages/06_regions.md) — Defining, saving and restoring regions
7. [Statistics widget](pages/07_statistics.md) — Extracting values out of CARTA 
8. [Guide on plotting Tools](pages/08_tools.md) — Overlaying contours, vectors, and catalogues 
9. [Basic Spectral analysis](pages/09_spectral_analysis.md) — Spectral line analysis, moments, and PV plots



---

## Useful links

- [CARTA website](https://cartavis.org)
- [ALMA Science Archive](https://almascience.eso.org/aq)
- [Italian ARC node](https://arc.ira.inaf.it/)


---

## Images used in this tutorial

This tutorial uses images extracted from the ALMA Science Archive from the ARI-L project (Massardi et al. 2021) for the following targets:

- **SDP9** projID: 2015.1.00415.S (PI. Wong)
ALMA Band 6 data (see also Massardi et al. 2018)

CO(6-5): https://almascience.eso.org/dataPortal/member.uid___A001_X2de_X28.ari_l.SDP9_sci.spw0_268325MHz.12m.cube.I.pbcor.fits

continuum: https://almascience.eso.org/dataPortal/member.uid___A001_X2de_X28.ari_l.SDP9_sci.spw0_1_2_3_259998MHz.12m.cont.I.pbcor.fits

This tutorial also uses some images from the **SHORES 2.1 GHz ATCA Survey** for an AGN target (courtesy of the SHORES collaboration, see also Massardi et al. 2025, Behiri et al. 2026 and Galluzzi et al. in preparation)

Useful files can be downloaded from the drive https://drive.google.com/drive/folders/13hIK4j3OcXc_kJvO--uVNGkBeYkEL1V5?usp=sharing